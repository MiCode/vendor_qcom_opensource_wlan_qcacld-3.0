...
#define ELF_BDF_FILE_NAME_N9		"bd_n9.elf"
#define ELF_BDF_FILE_NAME_N9_INDIA	"bd_n9in.elf"
#define ELF_BDF_FILE_NAME_N16T          "bd_n16t.elf"
#define ELF_BDF_FILE_NAME_N16T_INDIA    "bd_n16tin.elf"
#define ELF_BDF_FILE_NAME_N16T_GLOBAL	"bd_n16tgl.elf"
				   ...
				   u32 filename_len)
{
	...
	case ICNSS_BDF_ELF:
		if (priv->board_id == 0xFF)
		        if (hw_platform_ver == HARDWARE_PROJECT_N9) {
				if ((uint32_t)CountryIndia == hw_country_ver)
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_N9_INDIA);
                                else
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_N9);
			} else if (hw_platform_ver == HARDWARE_PROJECT_N16T) {
                                if ((uint32_t)CountryIndia == hw_country_ver)
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_N16T_INDIA);
				else if ((uint32_t)CountryGlobal == hw_country_ver)
				snprintf(filename_tmp, filename_len,
					ELF_BDF_FILE_NAME_N16T_GLOBAL);
				else
                                snprintf(filename_tmp, filename_len,
                                        ELF_BDF_FILE_NAME_N16T);
