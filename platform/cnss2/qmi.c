...
#define ELF_BDF_FILE_NAME_N1            "bd_n1.elf"
#define ELF_BDF_FILE_NAME_N1_GLOBAL     "bd_n1gl.elf"
#define ELF_BDF_FILE_NAME_N1_INDIA      "bd_n1in.elf"
#define ELF_BDF_FILE_NAME_N18            "bd_n18.elf"
#define ELF_BDF_FILE_NAME_N18_GLOBAL     "bd_n18gl.elf"
...
#define ELF_BDF_FILE_NAME_N8		"bd_n8.elf"
#define ELF_BDF_FILE_NAME_N8_GLOBAL	"bd_n8gl.elf"
#define ELF_BDF_FILE_NAME_N11U		"bd_n11u.elf"
#define ELF_BDF_FILE_NAME_N11U_GLOBAL	"bd_n11ugl.elf"
#define ELF_BDF_FILE_NAME_N801		"bd_n801.elf"
	...
	case CNSS_BDF_ELF:
		...
		if (plat_priv->board_info.board_id == 0xFF) {
			if (plat_priv->chip_info.chip_id & CHIP_ID_GF_MASK)
				...
				if (hw_platform_ver == HARDWARE_PROJECT_N2) {
					if ((uint32_t)CountryGlobal == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N2_GLOBAL);
					else if ((uint32_t)CountryIndia == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N2_INDIA);
					else
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N2);
				} else if (hw_platform_ver == HARDWARE_PROJECT_N3) {
					if ((uint32_t)CountryGlobal == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N3_GLOBAL);
					else if ((uint32_t)CountryIndia == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N3_INDIA);
					else
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N3);
				} else if (hw_platform_ver == HARDWARE_PROJECT_N8) {
                                        if ((uint32_t)CountryGlobal == hw_country_ver)
                                        snprintf(filename_tmp, filename_len,
                                                ELF_BDF_FILE_NAME_N8_GLOBAL);
                                        else
                                        snprintf(filename_tmp, filename_len,
                                                ELF_BDF_FILE_NAME_N8);
                                } else if (hw_platform_ver == HARDWARE_PROJECT_N11U) {
					if ((uint32_t)CountryGlobal == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N11U_GLOBAL);
					else
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N11U);
				} else if (hw_platform_ver == HARDWARE_PROJECT_N1) {
					if((uint32_t)CountryGlobal == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N1_GLOBAL);
					else if ((uint32_t)CountryIndia == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N1_INDIA);
					else
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N1);
				} else if (hw_platform_ver == HARDWARE_PROJECT_N801) {
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N801);
				} else if (hw_platform_ver == HARDWARE_PROJECT_N18) {
					if((uint32_t)CountryGlobal == hw_country_ver)
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N18_GLOBAL);
					else
					snprintf(filename_tmp, filename_len,
						ELF_BDF_FILE_NAME_N18);
				}
				else
				snprintf(filename_tmp, filename_len,
					 ELF_BDF_FILE_NAME);
